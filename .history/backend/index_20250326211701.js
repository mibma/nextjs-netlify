...
// ROUTES
app.use('/products', require("./products/productaymentRoutes"));
    ...
    db.run(
      ...
      function(err) {
        ...
        if (movement_type === 'SALE' || movement_type === 'MANUAL_REMOVAL') {
          // For sales/removals, we decrease inventory (quantity_change should be negative)
          updateQuery = `
            UPDATE inventory 
          ...
          // For stock in/adjustments, we increase inventory
          updateQuery = `
            UPDATE inventory 
            SET current_quantity = current_quantity + ?, 
                last_updated = CURRENT_TIMESTAMP
        ...
        db.run(
          updateQuery,
          [Math.abs(quantity_change), product_id, Math.abs(quantity_change)],
          function(updateErr) {
